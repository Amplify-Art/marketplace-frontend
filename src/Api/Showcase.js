...
export const getShowcases = (payload = {}) => {
  const url = makeUrl(`${API_ENDPOINT_URL}/showcases/`, { ...(payload.params || {}) });
...
export const getShowcaseById = (payload) => {
  const id = payload.id
  const url = makeUrl(`${API_ENDPOINT_URL}/showcases/${id}`, { ...(payload.params || {}) });
  return axios.get(url, {
    headers: {
      Authorization: `Bearer ${getAccessToken()}`
    },
...
export const addShowcase = (payload) => {
  const url = `${API_ENDPOINT_URL}/showcases`;
...
export const updateShowcase = (payload) => {
  const id = payload.id
  const url = `${API_ENDPOINT_URL}/showcases/${id}`;
