...
function SongList(props) {
  ...
  const onBuy = async () => {
    console.log(buyingSong, parseNearAmount(`${buyingSong.yocto_near_price}`), 'buying_song')
    if (user.near_account_type === 'connected') {
      ...
      await (props.wallet.account()).functionCall(
        process.env.REACT_APP_NEAR_MARKET_ACCOUNT || 'market.aatest.testnet',
        'offer',
        {
          nft_contract_id: process.env.REACT_APP_NFT_CONTRACT || 'nft.aatest.testnet',
